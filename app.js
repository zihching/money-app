...
function getFormData() {
    ...
    return { 
        ...
        type, category, collector, note, status, appointmentTime, // 加入 appointmentTime
...
window.openReportAction = function(mode, address, year, month, recordId, date, amount, type, floor, note, status) { 
    ...
    if(mode === 'edit') {
        ...
        content.innerHTML = ` 
            ...
            <div class="grid grid-cols-2 gap-2 mt-4"><button onclick="deleteReportRecord('${recordId}')" class="py-2 bg-red-100 text-red-600 rounded-lg font-bold">刪除紀錄</button><button onclick="updateReportRecord('${recordId}', document.getElementById('reportEditDate').value, document.getElementById('reportEditAmount').value, document.getElementById('reportEditType').value, document.getElementById('reportEditFloor').value, document.getElementById('reportEditNote').value, document.getElementById('reportEditStatus').value)" class="py-2 bg-blue-600 text-white rounded-lg font-bold">儲存修改</button></div>`; 
    } else { 
...
window.updateReportRecord = async function(docId, date, amount, type, floor, note, status) { 
    if(!currentUser) return; 
    try { 
...
window.renderPendingList = function() { 
    ...
    // 過濾並排序
    // 排序邏輯：有預約時間的排最上面 (時間越早越前)，沒有時間的排後面
    const items = window.appState.pending
        .filter(i => (i.collector === current) || (!i.collector && current === '子晴'))
        .sort((a, b) => {
            if (a.appointmentTime && !b.appointmentTime) return -1; // a 有時間，排前
            if (!a.appointmentTime && b.appointmentTime) return 1;  // b 有時間，排前
            if (a.appointmentTime && b.appointmentTime) {
                // 都有時間，比大小 (字串比較即可，因為格式是 ISO)
                return a.appointmentTime.localeCompare(b.appointmentTime);
            }
            return (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0); // 都沒時間，按建立順序
        });
    ...
    items.forEach(item => { 
        ...
        if (item.appointmentTime) {
            ...
            timeTag = `<div class="bg-red-100 text-red-600 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 mb-1 w-fit"><i class="fa-solid fa-clock"></i> 預約: ${timeStr}</div>`;
        }
        ...
        div.innerHTML = ` 
            ...
            <div class="space-y-2"> 
                ...
                <div class="flex gap-2 items-center"> 
                    ...
                    <button onclick="openConfirmCollectionModal('${item.id}', ${item.amount}, '${item.address}', '${item.category || 'stairs'}', '${item.serviceDate || ''}')" class="bg-green-500 text-white w-10 h-10 rounded-full shadow flex items-center justify-center active:scale-90 transition-transform flex-shrink-0"> <i class="fa-solid fa-check"></i> </button> 
                </div> 
            </div> 
