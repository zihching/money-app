...
window.importFavoritesToPending = async function() {
    ...
    const customers = window.appState.customers.filter(c => 
        (c.collector === current) || 
        (!c.collector && current === '子晴')
    );

    if(customers.length === 0) { alert(`沒有 ${current} 的常用客戶資料`); return; }
    
    if(confirm(`確定要將 ${customers.length} 個客戶加入待收清單嗎？`)) {
        try {
            let count = 0;
            ...
            window.showToast(`已匯入 ${count} 筆`);
            window.toggleView('entry');
        } catch(e) { console.error(e); window.showToast("匯入部分失敗"); }
    }
};
...
window.renderYearlyReport = function() {
    ...
    window.appState.customers.filter(c => (c.collector === current) || (!c.collector && current === '子晴')).forEach(c => addressSet.add(c.address));
...
window.openHistory = function(address) {
    ...
    if(history.length === 0) {
        list.innerHTML = '<div class="text-center text-gray-400 py-10">尚無此地址的紀錄</div>';
    } else {
        history.forEach(h => {
            const d = new Date(h.date);
            const dateStr = `${d.getFullYear()}/${d.getMonth()+1}/${d.getDate()}`;
            let typeText = '現金';
            if(h.type === 'transfer') typeText = '匯款';
            if(h.type === 'linepay') typeText = 'LinePay';
            if(h.type === 'dad') typeText = '匯給爸爸';
            ...
            row.innerHTML = `
                <div>
                    ...
                    <div class="text-xs text-blue-500">${h.months || '未填月份'}</div>
                </div>
        ...
        if(record.status === 'no_receipt') {
            statusHtml = `
                <div class="mt-2 bg-red-50 p-2 rounded-lg border border-red-200 flex justify-between items-center">
                    <span class="text-xs font-bold text-red-600"><i class="fa-solid fa-triangle-exclamation"></i> 待給收據</span>
                    <button onclick="updateRecordStatus('${record.id}', 'completed')" class="px-3 py-1 bg-red-500 text-white text-xs rounded-full shadow active:scale-95">已補單</button>
                </div>
            `;
            ...
            statusHtml = `
                <div class="mt-2 bg-orange-50 p-2 rounded-lg border border-orange-200 flex justify-between items-center">
                    <span class="text-xs font-bold text-orange-600"><i class="fa-solid fa-hourglass-half"></i> 待確認匯款</span>
                    <button onclick="updateRecordStatus('${record.id}', 'completed')" class="px-3 py-1 bg-orange-500 text-white text-xs rounded-full shadow active:scale-95">款項已入</button>
                </div>
            `;
        }
            ...
            <div class="flex justify-between items-start">
                <div class="flex-1 mr-2">
                    ...
                    <div class="text-sm text-blue-600 font-bold bg-blue-50 inline-block px-2 py-0.5 rounded border border-blue-100">
                        <i class="fa-regular fa-calendar-check mr-1"></i> ${record.months || '未填月份'}
                    </div>
                </div>
                <div class="text-right">
...
window.renderCustomerSettings = function() {
    ...
    list.innerHTML = '';
    if(customers.length === 0) { list.innerHTML = `<div class="text-center text-gray-400 text-xs py-2">尚未建立 ${current} 的常用客戶</div>`; return; }
    ...
    customers.forEach(c => {
        ...
        div.innerHTML = `
            <div class="text-sm">
                <div class="font-bold text-gray-800"><span class="mr-1">${catIcon}</span> ${c.address} <span class="text-gray-400 text-xs font-normal">${c.floor || '不固定'}</span></div>
...
window.renderCustomerSelect = function() {
    ...
    const current = window.appState.currentCollector;
    const customers = window.appState.customers.filter(c => 
        (c.collector === current) || (!c.collector && current === '子晴')
    );
...
window.updateSummary = function() {
    ...
    window.appState.records.forEach(r => {
        let col = r.collector;
        if(!col || (col !== '子晴' && col !== '子涵' && col !== '宗敬')) {
            col = '其他';
            if (r.collector === '我') col = '其他';
            if (r.collector === '妹') col = '其他';
            if (r.collector === '弟') col = '其他';
        }
    ...
    document.getElementById('categoryBreakdown').innerHTML = `
        <div class="bg-white p-3 rounded-lg border border-orange-200 text-center">
            <div class="text-xs text-orange-600 font-bold mb-1">🪜 洗樓梯 (全部)</div>
        ...
        <div class="bg-white p-3 rounded-lg border border-cyan-200 text-center">
            <div class="text-xs text-cyan-600 font-bold mb-1">💧 洗水塔 (全部)</div>
    ...
    if (pendingReceiptCount > 0 || pendingPaymentCount > 0) {
        ...
        if (pendingReceiptCount > 0) {
            warningContainer.innerHTML += `<div class="bg-red-100 text-red-800 p-3 rounded-lg text-sm font-bold flex items-center"><i class="fa-solid fa-triangle-exclamation mr-2"></i> 您有 ${pendingReceiptCount} 筆帳款還沒給收據！</div>`;
        }
        if (pendingPaymentCount > 0) {
            warningContainer.innerHTML += `<div class="bg-orange-100 text-orange-800 p-3 rounded-lg text-sm font-bold flex items-center"><i class="fa-solid fa-hourglass-half mr-2"></i> 您有 ${pendingPaymentCount} 筆匯款尚未確認入帳！</div>`;
        }
    } else {
...
window.exportData = function() {
    ...
    a.download = `雲端收費備份_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
};
...
window.printAllRecords = function() {
    ...
    let html = `
        <div class="print-title">清潔收費總報表</div>
        <div style="text-align:center; margin-bottom:10px;">列印日期：${dateStr}</div>
        
        <div class="print-summary">
            <div>
                <div style="font-size:12px;">本期總收入</div>
                <div style="font-size:16px; font-weight:bold;">$${totalAmount.toLocaleString()}</div>
            </div>
            <div>
                <div style="font-size:12px;">現金總額</div>
                <div style="font-size:16px; font-weight:bold;">$${totalCash.toLocaleString()}</div>
            </div>
            <div>
                <div style="font-size:12px;">匯款總額</div>
            ...
            <div>
                <div style="font-size:12px;">已匯給爸爸</div>
    ...
    records.forEach(r => {
        ...
        const cat = r.category === 'tank' ? '水塔' : '樓梯';
        let type = '現金';
        if(r.type === 'transfer') type = '匯款';
        if(r.type === 'linepay') type = 'LinePay';
        if(r.type === 'dad') type = '已匯爸';
        
        let note = r.note || '';
        if(r.status === 'no_receipt') note += ' (欠收據)';
        if(r.status === 'no_payment') note += ' (未入帳)';

        const collector = r.collector || '子晴';
...
window.openAddCustomerModal = function() {
    ...
    document.getElementById('customerModalTitle').innerHTML = '<i class="fa-solid fa-user-plus text-green-600"></i> 新增常用客戶';
    ...
    window.setEditCustCategory('stairs'); 
...
window.openEditCustomerModal = function(id, addr, amt, floor, cat) {
    ...
    document.getElementById('customerModalTitle').innerHTML = '<i class="fa-solid fa-pen-to-square text-blue-600"></i> 編輯常用客戶';
